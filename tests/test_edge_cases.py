...
def analyze_quality_score_handling():
    ...
    for qlen, left, right, desc in test_cases:
        ...
        print(f"  slice=[{slice_start}:{slice_end}], length={max(0, slice_end - slice_start)}")
...
def analyze_cigar_edge_cases():
    ...
    edge_cases = [
        ...
        ("Only deletions", [(2, 15)]),
        ("Mixed with zero-length ops", [(0, 10), (1, 0), (0, 5)]),  # Invalid but possible
        ("All soft clips", [(4, 30)]),
...
def check_reference_start_arithmetic():
    ...
    for i, cigar in enumerate(test_cigars, 1):
        ...
        for trim_q in [0, 3, 8, 15]:
            ...
            print(f"  trim_q={trim_q}: ref_advance={ref_advance}, consumed={consumed_ops}")
        print()
