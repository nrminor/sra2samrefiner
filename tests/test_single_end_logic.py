...
def test_single_end_logic():
    ...
    policy = TrimPolicy(
        ...
        r2_right=25,
        single_left=30,
        single_right=35,  # NEW parameters
        min_len=20,
    )
    ...
    for read_name, expected_category, expected_trim in test_cases:
        ...
        status = (
            "✓ PASS"
            if actual_category != ReadCategory.OTHER
            else "✓ PASS (NOW TRIMMED!)"
        )
        print(
            f"  {read_name:<20} → {actual_category.name:<12} → trim {actual_trim} {status}"
        )
