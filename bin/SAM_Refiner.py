                            ...
                            if C == "I":
                                ...
                                if args.nt_call == 1:
                                    for N in range(delPos, delPos + int(run_length)):
                                        try:
                                            ...
                                            nt_call_dict_dict[N] = {"A": 0, "T": 0, "C": 0, "G": 0, "N": 0, "-": 0}
                                ...
                                for ntPos in range(query_pos, query_pos + run_length):
                                    if query_seq[ntPos] != ref[1][refPos + ntPos - 1]:
                                        mutations.append(
                                            ref[1][refPos + ntPos - 1] + str(refPos + ntPos) + query_seq[ntPos]
                                        )
                                    if args.nt_call == 1:
                                        if query_seq[ntPos] not in ["A", "T", "C", "G", "N", "-"]:
                                            mut_nt = "N"
                                        else:
                ...
                if insert_position >= rf[0] and insert_position <= rf[1]:
                    ...
                    if run_length % 3 == 0:
                        if orfPos % 3 == 1:
                            ...
                            iProt += (
                                "insert(" + ref[3][orf]["AAs"][(orfPos - 1) // 3] + str(((orfPos - 1) // 3) + 1) + AA
                            )
                    else:
            ...
            if abund >= args.min_samp_abund:
                try:
                    ...
                    if args.AAreport == 1:
                        try:
                            seq_fh.write(
                                f"{splitseqs[0]} {col_reads[splitseqs[2]]['AA_sequence']} {splitseqs[1]}\t{seq_species[key]}\t{abund:.3f}\n"
                            )
                        except:
                            seq_fh.write(
                                f"{splitseqs[0]} {splitseqs[2]} {splitseqs[1]}\t{seq_species[key]}\t{abund:.3f}\n"
                            )
                    else:
                    ...
                    if args.AAcentered == 1 and args.AAreport == 1:
                        try:
                            AAseq_fh.write(
                                f"{splitseqs[0]} {col_reads[splitseqs[2]]['AA_centered']} {splitseqs[1]}\t{seq_species[key]}\t{abund:.3f}\n"
                            )
                        except:
                            if splitseqs[2] != "Reference":
                                print("Non Reference seq without AA centereed output : " + splitseqs[2])
        else:
            break
                                            ...
                                            if i > 0:
                                                if entry[i][1] > entry[i - 1][1]:
                                                    if "insert" in entry[i - 1][0]:
                                                        ...
                                                        mutntseq += ref[1][
                                                            entry[i - 1][1]
                                                            + len(entry[i - 1][0].split("-")[0].strip("0123456789"))
                                                            - 1 : entry[i][1] - 1
                                                        ]
                                                    else:
                        ...
                        if consensus[Pos][0] != ref[1][Pos - 1]:
                            ...
                            if mod == 0:
                                ...
                                try:
                                    ...
                                    codon = "NNN"
                            ntcall_lines["line"][Pos] += (
                                "\t" + aa_call(codon) + "\t" + singlet_codon_call(Pos, consensus[Pos][0], ref[1])[1]
                            )
                        else:
                            ntcall_lines["line"][Pos] += "\t\t"

                        if (nt_call_dict_dict[Pos][consensus[Pos][1]] >= args.min_count) and (
                            (nt_call_dict_dict[Pos][consensus[Pos][1]] / total) >= args.min_samp_abund
                        ):
                            ntcall_lines["variant"][Pos] = 1
                            ntcall_lines["line"][Pos] += (
                            ...
                            if (nt_call_dict_dict[Pos][consensus[Pos][2]] >= args.min_count) and (
                                nt_call_dict_dict[Pos][consensus[Pos][2]] / total >= args.min_samp_abund
                            ):
                                ntcall_lines["variant"][Pos] = 1
                                ntcall_lines["line"][Pos] += (
                ...
                for Pos in sorted_Pos:
                    ...
                    if (total >= (sam_read_count * args.ntabund) or args.wgs == 1) and total >= args.ntcover:
                        ...
                        if ref[1][Pos - 1] != sorted_calls[0]:
                            ntcall_lines["variant"][Pos] = 1
                        if (nt_call_dict_dict[Pos][sorted_calls[1]] >= args.min_count) and (
                            ...
                            if (nt_call_dict_dict[Pos][sorted_calls[2]] > args.min_count) and (
                                nt_call_dict_dict[Pos][sorted_calls[2]] / total > args.min_samp_abund
                            ):
                                ntcall_lines["variant"][Pos] = 1
                                ntcall_lines["line"][Pos] += (
                                    "\t"
...
def gb_sam_parse(args, ref, file):
    ...
    if sam_read_count == 0:
        ...
        if args.AAreport == 1 and (args.seq == 1 or args.covar == 1 or args.indel == 1 or args.read == 1):
            for SNP_sequence in col_reads:
                if SNP_sequence == "Reference":
                    ...
                    args.AAcodonasMNP == 1
                ):  # similar to fasta ref processing, but performs aa checks for each reading frame
                    try:
                        col_reads[SNP_sequence]["AA_sequence"]
                    except:
                        mutorfs = {}
                                        ...
                                        if nt_to_AA_key in nt_to_AA_dict[orf]:
                                            ...
                                            for i in range(len(entry)):
                                                ...
                                                if i > 0:
                                                    if entry[i][1] > entry[i - 1][1]:
                                                        if "insert" in entry[i - 1][0]:
                                                            mutntseq += ref[3][orf]["nts"][
                                                                entry[i - 1][1] - 1 : entry[i][1] - 1
                                                            ]
                                                        elif "del" in entry[i - 1][0]:
                                                            mutntseq += ref[3][orf]["nts"][
                                                                entry[i - 1][1]
                                            ...
                                            if startmod == 2:
                                                ...
                                                wtorfstartpos = orfstartpos - 2
                                                mutntseq = (
                                                    ref[3][orf]["nts"][orfstartpos - 3 : orfstartpos - 1] + mutntseq
                                                )
                                            if endmod == 2:
                                                wtorfendpos = orfendpos + 1
                                                try:
        ...
        if args.nt_call == 1:
            ...
            if args.AAreport == 1:
                ntcall_fh.write(
                    "Position\tref NT\tAAs\tA\tT\tC\tG\t-\tN\tTotal\tPrimary NT\tCounts\tAbundance\tPrimary Seq AA\tsingle nt AA\tSecondary NT\tCounts\tAbundance\tAA\tTertiary NT\tCounts\tAbundance\tAA\n"
                )
                if args.ntvar == 1:
                    ntcallv_fh.write(
                        "Position\tref NT\tAAs\tA\tT\tC\tG\t-\tN\tTotal\tPrimary NT\tCounts\tAbundance\tPrimary Seq AA\tsingle nt AA\tSecondary NT\tCounts\tAbundance\tAA\tTertiary NT\tCounts\tAbundance\tAA\n"
                ...
                for orf in ref[3]:
                    ...
                    for rf in ref[3][orf]["reading frames"]:
                        for i in range(rf[0], rf[1] + 1):
                            ...
                            try:
                                OrfPosDict[i].append(
                                    [orf, orfPos, ref[3][orf]["AAs"][(orfPos - 1) // 3], (((orfPos - 1) // 3) + 1)]
                                )
                            except:
                                OrfPosDict[i] = [
                                    [orf, orfPos, ref[3][orf]["AAs"][(orfPos - 1) // 3], (((orfPos - 1) // 3) + 1)]
                    ...
                    if (total >= (sam_read_count * args.ntabund) or args.wgs == 1) and total >= args.ntcover:
                        ...
                        try:
                            ...
                            for entry in OrfPosDict[Pos]:
                                orfAAreports.append(
                                    entry[0] + "_nt:" + str(entry[1]) + "_AA:" + entry[2] + str(entry[3])
                                )
                        ...
                        if consensus[Pos][0] != ref[1][Pos - 1]:
                            ...
                            try:
                                ...
                                if (nt_call_dict_dict[Pos][consensus[Pos][1]] > args.min_count) and (
                                    ...
                                    if (
                                        nt_call_dict_dict[Pos][consensus[Pos][2]] > args.min_count
                                        and nt_call_dict_dict[Pos][consensus[Pos][2]] / total > args.min_samp_abund
                                    ):
                                        ntcall_lines["line"][Pos] += (
                                            f"\t\t{consensus[Pos][2]}\t{nt_call_dict_dict[Pos][consensus[Pos][2]]}\t{(nt_call_dict_dict[Pos][consensus[Pos][2]]/total):.3f}"
                                        )
                            else:
                                orfAAreports = [[], []]
                                for ORFentry in OrfPosDict[Pos]:
                                    ...
                                    orfAAreports[1].append(
                                        orf + "_" + singlet_codon_call(OrfPos, consensus[Pos][0], ref[3][orf]["nts"])[1]
                                    )
                                ntcall_lines["line"][Pos] += (
                                    "\t" + ", ".join(orfAAreports[0]) + "\t" + ", ".join(orfAAreports[1])
                                )
                                if (nt_call_dict_dict[Pos][consensus[Pos][1]] > args.min_count) and (
                                    nt_call_dict_dict[Pos][consensus[Pos][1]] / total > args.min_samp_abund
                                ):
                                    ...
                                    for ORFentry in OrfPosDict[Pos]:
                                        ...
                                        orfAAreports.append(
                                            orf
                                            + "_"
                                            + singlet_codon_call(OrfPos, consensus[Pos][1], ref[3][orf]["nts"])[1]
                                        )
                                    ntcall_lines["line"][Pos] += "\t" + ", ".join(orfAAreports)
                                        ...
                                        if nt_call_dict_dict[Pos][consensus[Pos][2]] / total > args.min_samp_abund:
                                            ...
                                            for ORFentry in OrfPosDict[Pos]:
                                                ...
                                                orfAAreports.append(
                                                    orf
                                                    + "_"
                                                    + singlet_codon_call(OrfPos, consensus[Pos][2], ref[3][orf]["nts"])[
                                                        1
                                                    ]
                                                )
                                            ntcall_lines["line"][Pos] += "\t" + ", ".join(orfAAreports)

                        elif (nt_call_dict_dict[Pos][consensus[Pos][1]] >= args.min_count) and (
                            (nt_call_dict_dict[Pos][consensus[Pos][1]] / total) >= args.min_samp_abund
                        ):
                            ntcall_lines["variant"][Pos] = 1
                            ...
                            try:
                                ...
                                for ORFentry in OrfPosDict[Pos]:
                                    ...
                                    orf = ORFentry[0]
                                    orfAAreports.append(
                                        orf + "_" + singlet_codon_call(OrfPos, consensus[Pos][1], ref[3][orf]["nts"])[1]
                                    )
                                ntcall_lines["line"][Pos] += "\t" + ", ".join(orfAAreports)
                                if nt_call_dict_dict[Pos][consensus[Pos][2]] > args.min_count:
                                    if nt_call_dict_dict[Pos][consensus[Pos][2]] / total > args.min_samp_abund:
                                        ...
                                        for ORFentry in OrfPosDict[Pos]:
                                            ...
                                            orfAAreports.append(
                                                orf
                                                + "_"
                                                + singlet_codon_call(OrfPos, consensus[Pos][2], ref[3][orf]["nts"])[1]
                                            )
                                        ntcall_lines["line"][Pos] += "\t" + ", ".join(orfAAreports)
                ...
                for Pos in sorted_Pos:
                    ...
                    if (total >= (sam_read_count * args.ntabund) or args.wgs == 1) and total >= args.ntcover:
                        ...
                        ntcall_lines["line"][Pos] += (
                            "\t" + str(nt_call_dict_dict[Pos]["-"]) + "\t" + str(nt_call_dict_dict[Pos]["N"])
                        )
                        ntcall_lines["line"][Pos] += (
                            "\t"
                            + str(total)
                        ...
                        if (nt_call_dict_dict[Pos][sorted_calls[1]] > args.min_count) and (
                            nt_call_dict_dict[Pos][sorted_calls[1]] / total > args.min_samp_abund
                        ):
                            ntcall_lines["line"][Pos] += (
                                "\t"
                                + sorted_calls[1]
